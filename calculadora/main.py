...
def entrar_valores(event):
    ...
    resultado = resultado + str(event)
    
    valor_texto.set(resultado)

def calcular():
    global resultado
    resultados = eval(resultado)
    
    valor_texto.set(str(resultados))
...
b_1.place(x=0,y=0)
b_2 = Button(frame_corpo,command = lambda: entrar_valores('%'), text="%", width=11, height=2, bg = cor4, font=('Ivy 13 bold'), relief=RAISED,overrelief=RIDGE)
b_2.place(x=90,y=0)
b_3 = Button(frame_corpo,command = lambda: entrar_valores('/'), text="/", width=11, height=2, bg = cor5, font=('Ivy 13 bold'), relief=RAISED,overrelief=RIDGE)
b_3.place(x=170,y=0)
